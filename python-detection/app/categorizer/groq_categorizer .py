...
def classify_groq(evidence: List[dict]) -> CategoryResult:
    ...
    try:
        ...
        raise CategorizerUnavailableError(f"Groq request timed out after {_REQUEST_TIMEOUT_SECONDS}s") from exc
    except groq.APIConnectionError as exc:
        raise CategorizerUnavailableError("Could not reach Groq API") from exc
    except groq.APIStatusError as exc:
        # Reached Groq, but got a non-2xx (bad API key, bad model name,
        # rate limit, etc.) — surfaced distinctly from a malformed-but-
        # received chat response, same "transport vs shape" split as
        # classify_local's ollama.ResponseError handling.
        raise CategorizerUnavailableError(f"Groq returned an error: {exc}") from exc
